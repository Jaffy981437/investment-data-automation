...
### 2026-08-21 — ETF 情報補齊:上網查證三檔成分股

**任務**:承接上一份截圖分析,上網收集 00830 / 00876 / 00911 的最新成分股情報。

**產出**:`reports/半導體ETF情報更新.html`(不進版控)
+ Artifact <https://claude.ai/code/artifact/537760a6-a15c-47e3-a00d-c8725c1b34d1>
+ 已上傳 Google Drive `13_投資/ETF分析/半導體ETF情報更新.html`
  (檔案 ID `18Z77nKRdn1-Xaa1IBbW5m_uPnUo0Xk7T`,會同步回 `G:\`)

**環境限制(重要,下次別重踩)**:

本 session 的網路出口代理**擋掉所有台灣投信與財經網站**,WebFetch 與 curl 皆回 403:
`cathaysite.com.tw`、`megafunds.com.tw`、`yuantaetfs.com`、`moneydj.com`、
`twse.com.tw`,連 `nasdaq.com`、`stockanalysis.com` 也不通。**只有 WebSearch 可用**。

→ 上次「成分股權重要從投信官網原始持股檔抓」的結論**方向正確,但不能在這個環境執行**。
   要落地就得放到沒有 egress 限制的地方 —— **GitHub Actions**(專案已在用,`fetch_prices.py` 就跑在那)。
   這是下次要寫 `fetch_holdings.py` 時的架構前提。

**補到的新資料**(上次列為「拿不到」的五項補齊四項):

| | 00830 | 00876 | 00911 |
|---|---|---|---|
| 追蹤指數 | 費城半導體 SOX(30檔) | iSTOXX 全球5G關鍵科技 | 洲際半導體 NYSE/ICE(30檔) |
| 權重上限 | **階梯 12/10/8%,其餘約4%** | 齊頭 <10% | 齊頭 ≤10% |
| 經理費+保管費 | 0.45→0.35% + 0.11→0.06%(階梯) | 0.90% + 0.23% = **1.13%** | 0.85% + 0.20% = **1.05%** |
| 基金規模 | 705.59 億 | 64.46 億(05/22) | **8.3 億(04/23) / 11.88 億(06/15)** |
| 受益人數 | 逾 10 萬人 | 未查得 | 6,575 人(04/24) |
| 配息 | 年配,2026 配 9 元 | **近5年未配息** | 2026/01 配 1.00 元 |

**最有價值的發現:集中度差異是指數規則決定的,不是投信選的。**
SOX 的階梯式上限(12/10/8)讓龍頭拉開差距 → 00830 的 NVIDIA 13.65%;
ICE 與 iSTOXX 是齊頭 10% 上限 → 00911 被壓平成 9.04/8.47/7.95/7.15。
**規則不會每天變,權重會** —— 所以規則比權重快照可靠。
(註:13.65% 超過 12% 上限不是錯誤,上限只在再平衡時套用,期間會漂移。)

**兩處對上一份報告的修正**:

1. 00830 **第 10 大持股就是 AMD**,不是「沒有超微」。差別在權重大小(00911 放第 1 大)。
   真正 00830 沒有、00911 有的是**英特爾**。
2. 00876 第 8–10 名是 **Murata、Qualcomm、Shin-Etsu** —— Qualcomm 是 5G 基頻晶片龍頭。
   「幾乎沒有 5G 成分」的說法過重,正確說法是「前段由設備股主導,5G 名字排在第 8–10」。

**資料衝突(未採用,列出供下次參考)**:
SOX 權重三個來源三種答案(NVDA 13.65% / 9.0% / 5.88%),差距大到不可能只是日期造成。
推測是混淆了 SOX 指數本身、美國 SOXX ETF、台灣 00830 三種不同標的。
**因此報告不提供「最新權重表」—— 沒有可信來源時空著比填錯好。**

**淨效果**:三個月選誰的結論不變(00911 彈性最大 / 00830 報酬風險比最佳),
但 00911 的流動性判斷要**往保守修正** —— 規模僅 8–12 億、受益人 6,575 人,
折溢價風險比單看日均量估出來的高。
